...
from src.utils.mlflow import get_exps_metric, get_site_time_results, get_site_pred_time
import numpy as np
import pandas as pd
...
def plot_fusion_times(results, times, pred_times, site_code):
   sns.set_theme(style="darkgrid")
   ...
   new_results['full_time'] = new_results['Epochs'] * new_results['time'] * 5 * 20 / 3600 # each epoch have 200 batches and the time is referred to 10 batches
   ...
   #ax.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
   plt.suptitle(f'Average Training Times - Fusion Models')
   plt.xticks(ha='left')
   ...
   #new_results = new_results[new_results['model'] == 'model_0']
   
   sns.set_theme(font_scale=1.5)
   sns.set_palette('tab10')
   fig, ax = plt.subplots(figsize=(10,5))
   sns.barplot(data = new_results, x ='Base Architecture', y = 'time', hue = 'Model', ax = ax, edgecolor='k', errorbar = 'sd', err_kws = {'alpha': 0.5}, capsize=0.05, gap=0.05, estimator='mean', legend = True)
   sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
   plt.setp(ax.get_legend().get_texts(), fontsize='10') 
   plt.setp(ax.get_legend().get_title(), fontsize='12') 
   #ax.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
   plt.suptitle(f'Average Prediction Times - Fusion Models')
   plt.xticks(ha='left')
   ...
   plt.tight_layout()
   st.pyplot(fig)
   plt.savefig(f'figures/eval-time-fusion.png', dpi=300, bbox_inches='tight')
   plt.close(fig)
...
epochs, times, pred_times = None, None, None
for site_code in st.session_state['sites']:
   sns.set_theme(style="darkgrid")
   ...
   exps = [101, 301, 411, 412, 413, 103, 151, 351, 461, 462, 463, 153]
   exp_codes = [f'exp_{code}' for code in exps]
   ...
   if pred_times is None:
      pred_times = get_site_pred_time(site_name, st.session_state['experiments'], exp_codes)
   else:
      pred_times = pd.concat([pred_times, get_site_pred_time(site_name, st.session_state['experiments'], exp_codes)]) 
   
   
plot_fusion_times(epochs, times, pred_times, site_code)