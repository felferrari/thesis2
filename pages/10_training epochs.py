...
def plot_siamese_epochs_com(results, site_code):
   ...
   sns.barplot(data = opt_results, x ='Base Architecture', y = 'Epochs', hue = 'Temporal Aggregation', ax = ax, edgecolor='k', errorbar = 'sd', err_kws = {'alpha': 0.5}, capsize=0.05, gap=0.05, estimator='mean', legend = True)
   ...
   sns.set_palette('tab10')
   fig, ax = plt.subplots(figsize=(12,5))
   sns.barplot(data = sar_results, x ='Base Architecture', y = 'Epochs', hue = 'Temporal Aggregation [SAR Dataset]', ax = ax, edgecolor='k', errorbar = 'sd', err_kws = {'alpha': 0.5}, capsize=0.05, gap=0.05, estimator='mean', legend = True)
   sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
...
def plot_prevmap_epochs_com(results, site_code):
   ...
   sns.barplot(data = opt_results, x ='Base Architecture', y = 'Epochs', hue = 'Previous Map', ax = ax, edgecolor='k', errorbar = 'sd', err_kws = {'alpha': 0.5}, capsize=0.05, gap=0.05, estimator='mean', legend = True)
   sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
   plt.setp(ax.get_legend().get_texts(), fontsize='12') 
   plt.setp(ax.get_legend().get_title(), fontsize='14') 
   #ax.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
   ...
   sns.set_palette('tab10')
   fig, ax = plt.subplots(figsize=(10,6))
   sns.barplot(data = sar_results, x ='Base Architecture', y = 'Epochs', hue = 'Previous Map', ax = ax, edgecolor='k', errorbar = 'sd', err_kws = {'alpha': 0.5}, capsize=0.05, gap=0.05, estimator='mean', legend = True)
   sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
   plt.setp(ax.get_legend().get_texts(), fontsize='12') 
   plt.setp(ax.get_legend().get_title(), fontsize='14') 
...
def plot_fusion_size_com(results, site_code):
   sns.set_theme(style="darkgrid")
   ...
   opt_results = opt_results.groupby(['full_name', 'Base Architecture', 'Previous Map', 'Model', 'model']).size().reset_index(name='Epochs')
   
   sns.set_theme(font_scale=1.5)
   sns.set_palette('tab10')
   fig, ax = plt.subplots(figsize=(10,6))
   sns.barplot(data = opt_results, x ='Base Architecture', y = 'Epochs', hue = 'Model', ax = ax, edgecolor='k', errorbar = 'sd', err_kws = {'alpha': 0.5}, capsize=0.05, gap=0.05, estimator='mean', legend = True)
   sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
   plt.setp(ax.get_legend().get_texts(), fontsize='10') 
   plt.setp(ax.get_legend().get_title(), fontsize='12') 
   #ax.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
   plt.suptitle(f'Training Epochs - Fusion Models - Site {site_code[-1]}')
   plt.xticks(ha='left')
   # plt.title(f'Average Training Time (10 Epochs) - CLOUD-FREE', pad=35, fontsize=24)
   # plt.xticks(ha='center')
   #plt.yticks(np.linspace(0.3, 1.0, 8))
   ...
   plt.tight_layout()
   st.pyplot(fig)
   plt.savefig(f'figures/epochs-fusion-{site_code}.png', dpi=300, bbox_inches='tight')
   plt.close(fig)
...
for site_code in st.session_state['sites']:
   ...
   plot_fusion_size_com(epochs_results, site_code)