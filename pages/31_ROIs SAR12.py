   ...
   for roi in rois:
      ...
      for exp in exps:  
         ...
         for comb in combs:
            ...
            full_image = Image.new('RGB', (5*600,2*1200))
            ...
            for i in range(4):
               for j in range(3):
                  ...
                  draw.text((350+j*600, (i*600)+25), f"{3*i+j}", (0,0,0), font = font2, anchor ='mt')
               
            img_name = f"ref_{site_name}-{experiments[exp]['name']}-{comb.split('_')[1]}_roi_{roi.split('_')[1]}"
            full_image.paste(images[exp][roi][comb][img_name], (3*600, 0*1200))
            
            img_name = f"def_class_{site_name}-{experiments[exp]['name']}-{comb.split('_')[1]}_roi_{roi.split('_')[1]}"
            full_image.paste(images[exp][roi][comb][img_name], (3*600, 1*1200))
            
            draw.text((4*1200/2, 30), "Reference", (0,0,0), font = font1, anchor ='mt')
            draw.text((4*1200/2, 1230), "Prediction", (0,0,0), font = font1, anchor ='mt')
