...
def plot_opt_rois(site_name, experiments, images, rois_codes, save, font_size):
   ...
   for roi in rois:
      ...
      for comb in combs:
         ...
         full_image = Image.new('RGB', ((1+len(exps))*1200,1200))
         
         img_name = f"ref_{site_name}-{experiments[exps[0]]['name']}-{comb.split('_')[1]}_roi_{roi.split('_')[1]}"
         full_image.paste(images[exps[0]][roi][comb][img_name], (0*1200, 0*1200))
         
         draw = ImageDraw.Draw(full_image)
         draw.text((1200/2, 30), "Reference", (0,0,0), font = font, anchor ='mt')
         
         for i, exp in enumerate(exps):
            if len(images[exp][roi]) == 1:
               img_name = f"def_class_{site_name}-{experiments[exp]['name']}-0_roi_{roi.split('_')[1]}"
               full_image.paste(images[exp][roi]['comb_0'][img_name], ((1+i)*1200, 0*1200))
            else:
               img_name = f"def_class_{site_name}-{experiments[exp]['name']}-{comb.split('_')[1]}_roi_{roi.split('_')[1]}"
               full_image.paste(images[exp][roi][comb][img_name], ((1+i)*1200, 0*1200))
            mname = experiments[exp]['full_name'].split(' ')[1]
            draw.text(((3+2*i)*1200/2, 30), mname, (0,0,0), font = font, anchor ='mt')
         
         
         if save:
            full_image.save(f'figures/sample-results-{exps}-{roi}-{comb}.png')
         st.image(full_image)
...
exps_l = [exp_k for exp_k in st.session_state['experiments']]
...
#       plot_opt_rois(site_name, st.session_state['experiments'], images, rois_codes, save, font_size)
