   ...
   for roi in rois:
      ...
      for exp in exps:  
         ...
         for comb in combs:
            ...
            img_name = f"sar_{site_name}-{experiments[exp]['name']}-{comb.split('_')[1]}_roi_{roi.split('_')[1]}_sar_0"
            full_image.paste(resize_img(images[exp][roi][comb][img_name]), (0*1200, 0*1200))
            
            if f"sar_{site_name}-{experiments[exp]['name']}-{comb.split('_')[1]}_roi_{roi.split('_')[1]}_sar_11" in list(images[exp][roi][comb].keys()):
               img_name = f"sar_{site_name}-{experiments[exp]['name']}-{comb.split('_')[1]}_roi_{roi.split('_')[1]}_sar_11"
            else:
               img_name = f"sar_{site_name}-{experiments[exp]['name']}-{comb.split('_')[1]}_roi_{roi.split('_')[1]}_sar_1"
            ...
            draw.text((1200/2, 30), "SAR First", (0,0,0), font = font, anchor ='mt')
            draw.text((3*1200/2, 30), "SAR Last", (0,0,0), font = font, anchor ='mt')
            draw.text((5*1200/2, 30), "Reference", (0,0,0), font = font, anchor ='mt')
            draw.text((7*1200/2, 30), "Prediction", (0,0,0), font = font, anchor ='mt')
            
            if save:
               full_image.save(f'figures/sample-sar-pred-{exp}-{roi}-{comb}.png')
            st.image(full_image)
...
exps_l = [exp_k for exp_k in st.session_state['experiments'] if st.session_state['experiments'][exp_k]['sar_condition'] != 'no_sar']
...
#       plot_sar_rois(site_name, st.session_state['experiments'], images, rois_codes, save, font_size)
